...
def load(path, name):
    try:
        ...
        raise Exception(f'The font file by the path "{path}" was not found') from None
    except ValueError:
        raise Exception(f'The font file by the path "{path}" has incorrect format') from None
    flist = []
    element = ""
    i = 0
    for line in content[1:]:
        ...
        if i == height:
            ...
            element = ""
    if len(flist) < 162:
        raise Exception(f'The font file by the path "{path}" has not enough data')
    fonts[name] = flist
...
def render(text, name):
    ...
    try:
        ...
        raise Exception(f'The font "{name}" was not loaded') from None
    split = font[indexes[""]]
    result = split
